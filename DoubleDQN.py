...
LR = 5e-5
SAVE_PATH = './atari_model_DoubleDQN_Breakout{0}.pack'.format(LR)
SAVE_INTERVAL = 10000
LOG_DIR = './logs/atari_DoubleDQN_Breakout' + str(LR)
LOG_INTERVAL = 1000
    ...
    def compute_loss(self, transitions, target_net):
        ...
        with torch.no_grad():
            if self.double:
                ...
                targets_online__best_q_indices = targets_online_q_values.argmax(dir=1, keepdim=True)

                targets_target_q_values = target_net(new_obses_t)
                targets_selected_q_values = torch.gather(input=targets_target_q_values, \
                                                         dim=1, index=targets_online__best_q_indices)
            else:
